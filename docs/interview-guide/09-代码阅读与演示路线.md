...
| 想深入的问题 | 先读源码 | 再读学习章 |
| --- | --- | --- |
| Context 为什么不是整库 dump | `v11_runtime.py` 的 `_live_*_projection`、`_select_evidence_digest` | [16-上下文工程](16-上下文工程与压缩.md) |
| Tool 怎样抵抗重试/越权 | `adaptive_tools.py`、`tool_queries.py`、`provider_tools.py` | [17-Tool Calling](17-工具调用生产落地.md) |
| 历史经验怎样安全复用 | `memory/store.py`、`VerifiedMemoryLookup` | [18-Memory 与 RAG](18-智能体记忆与检索增强生成.md) |
| endpoint 怎样准入 | `model_capability.py`、`openai_compatible_model.py` | [20-模型路由与能力](20-模型路由能力准入与安全降级.md) |
| 一次 Run 怎样观测/评测 | `telemetry.py`、`event_hub.py`、`benchmarks/rcaeval` | [21-观测](21-智能体可观测性与调试.md)、[23-评测](23-智能体评测科学.md) |
